...
const EMBED_LOCK_ENABLED = (env.EMBED_LOCK_ENABLED || "true").toLowerCase() === "true";
const SESSION_SECONDS = 180;
const SKEW_SECONDS = 30;
type JwtPayload = { iss?: string; aud?: string; sid?: string; iat?: number; exp?: number; jti?: string; };
...
async function verifyToken(token: string): Promise<JwtPayload> {
    ...
    return ( pathname === "/health" || pathname === "/favicon.ico" || pathname === "/robots.txt"
...
export async function proxy(req: NextRequest) {
    ...
    try {
        ...
        if (!sid) { return forbidden("Invalid portfolio session.");
        ...
        if (!queryToken && cookieSid && cookieSid !== sid) {
            ...
            res.cookies.set(TOKEN_COOKIE, token, {httpOnly: true, secure: true, sameSite: "none", path: "/", maxAge: SESSION_SECONDS,
            });

            res.cookies.set(SID_COOKIE, sid, {httpOnly: true, secure: true, sameSite: "none", path: "/", maxAge: SESSION_SECONDS, });
