...
echo "[init-postgres] Checking if schema 'ss' already exists..."
if psql "$DATABASE_URL" -tAc "SELECT 1 FROM pg_namespace WHERE nspname='ss' LIMIT 1;" | grep -q 1; then
  echo "[init-postgres] Schema 'ss' already exists. Skipping (only runs on fresh DB)."
  exit 0
fi
